...
async function init() {
  try {
    const [{ data: c, error: ce }, { data: t, error: te }, { data: f, error: fe }] = await Promise.all([
...
function renderDashboard() {
  ...
  const tablaIVA = t => `
    <table class="iva-table">
      ...
      <tr class="iva-total"><td>Total IVA</td><td>Gs. ${fmt(t.iva)}</td></tr>
    </table>`
...
function renderClientesGrid() {
  ...
  if (!clientes.length) {
    el.innerHTML = '<p class="empty-state">No hay clientes aún. Creá el primero.</p>'
    return
  }
...
function renderPerfilHistorial() {
  if (!clienteActivo) return
  const fs = facturas.filter(f => f.cliente_id === clienteActivo.id).sort((a, b) => b.fecha.localeCompare(a.fecha))
  const el = document.getElementById('perfil-historial')
  if (!fs.length) {
    el.innerHTML = '<p class="empty-state">No hay facturas. Usá la pestaña "Nueva factura" para agregar.</p>'
    return
  }
...
function toggleTimbradoPerfil() {
  document.getElementById('pf-timbrado-inline').classList.toggle('visible')
}
...
async function guardarFacturaPerfil() {
  ...
  if (totalInput && Math.abs(totalInput - distribuido) > 1) {
    if (!confirm('El total distribuido no coincide. ¿Guardar igual?')) return
  }
  const payload = {
...
function renderTimbrados() {
  ...
  el.innerHTML = `<div class="table-wrap"><table>
    ...
    return `<tr>
        ...
        <td style="font-size:12px">${t.inicio}</td>
        <td style="font-size:12px">${t.fin}</td>
...
function renderFacturas() {
  ...
  el.innerHTML = `<div class="table-wrap"><table>
    <thead><tr>
      <th>N° Factura</th><th>Tipo</th><th>Fecha</th><th>Cliente</th>
      <th style="text-align:right">Grav.10%</th><th style="text-align:right">Grav.5%</th>
      <th style="text-align:right">Exenta</th><th style="text-align:right">IVA</th>
      <th style="text-align:right">Total</th><th></th>
    </tr></thead>
    ...
    return `<tr>
        ...
        <td style="font-size:12px">${f.fecha}</td>
        <td style="max-width:140px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${f.nombre || '—'}</td>
        <td style="text-align:right">${f.g10 ? fmt(f.g10 - iva10(f.g10)) : '—'}</td>
        <td style="text-align:right">${f.g5 ? fmt(f.g5 - iva5(f.g5)) : '—'}</td>
        <td style="text-align:right">${f.exenta ? fmt(f.exenta) : '—'}</td>
        <td style="text-align:right">${fmt(c.ivaTotal)}</td>
  ...
  const tot = list.reduce((a, f) => {
    const c = calcIVA(f)
    return { g10: a.g10 + (f.g10 || 0), g5: a.g5 + (f.g5 || 0), exenta: a.exenta + (f.exenta || 0), iva: a.iva + c.ivaTotal, total: a.total + c.total }
  }, { g10: 0, g5: 0, exenta: 0, iva: 0, total: 0 })
  document.getElementById('resumen-filtrado').innerHTML = `
    <div class="resumen-filtrado">
